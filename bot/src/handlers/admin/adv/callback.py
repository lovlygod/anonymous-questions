...
async def admin_adv_initial(callback_query: CallbackQuery, bot: Bot, db: MongoDbClient):
    await callback_query.answer('Реклама')  # Send a response to the callback query
    ...
    builder.row(InlineKeyboardButton(text='Редактировать пост', callback_data=AdvEdit(
    ...
    if adv_quantity > 1:
        builder.add(InlineKeyboardButton(text='Следующий', callback_data=AdvNav(
            index=next_adv_query.adv_id).pack()))  # If there are more than one post, add "Next" button
    builder.row(InlineKeyboardButton(text='Добавить пост', callback_data=AddAdv().pack()))  # Add "Add Post" button
    builder.add(InlineKeyboardButton(text='Удалить пост', callback_data=AdvRemove(
...
async def add_adv(callback_query: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    msg = await bot.send_message(chat_id=callback_query.from_user.id,
                                 text='Отправьте рекламный пост')  # Ask the user to send the advertisement post
...
async def admin_adv_navigation(callback_query: CallbackQuery, bot: Bot, db: MongoDbClient, callback_data: AdvNav):
    ...
    if not adv_query:
        await bot.send_message(callback_query.from_user.id,
                               text='Рекламные посты не найдены.')  # If the post is not found, send a message
        return
    ...
    if prev_adv_query:
        builder.row(InlineKeyboardButton(text='Назад', callback_data=AdvNav(
...
async def adv_edit_choose(callback_query: CallbackQuery, callback_data: AdvEdit, bot: Bot):
    await callback_query.answer('Редактировать')  # Send a response to the callback query
    ...
    keyboard.row(InlineKeyboardButton(text='Редактировать текст',
                                      callback_data=AdvTextEdit(adv_id=adv_id).pack()))  # Add "Edit Text" button
    keyboard.row(InlineKeyboardButton(text='Редактировать медиа',
                                      callback_data=AdvMediaEdit(adv_id=adv_id).pack()))  # Add "Edit Media" button
    keyboard.row(InlineKeyboardButton(text='Назад', callback_data=AdminAdv().pack()))  # Add "Back" button
...
async def adv_text_edit_start(callback_query: CallbackQuery, callback_data: AdvTextEdit, bot: Bot, state: FSMContext):
    await callback_query.answer('Текст')  # Send a response to the callback query
    adv_id = int(callback_data.adv_id)  # Get the advertisement post ID
    msg = await bot.send_message(
        chat_id=callback_query.from_user.id,
        text='Введите новый текст (поддерживаются HTML теги):'
...
async def adv_media_edit_start(callback_query: CallbackQuery, callback_data: AdvMediaEdit, bot: Bot, state: FSMContext):
    await callback_query.answer('Медиа')  # Send a response to the callback query

    adv_id = int(callback_data.adv_id)  # Get the advertisement post ID
    msg = await bot.send_message(
        chat_id=callback_query.from_user.id,
        text='Отправьте новый медиа файл:')  # Ask the user to send the new media
